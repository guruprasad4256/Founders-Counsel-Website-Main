...
export default function Navbar(): JSX.Element {
  ...
  const isActive = (path: string): string =>
    location.pathname === path
      ? "text-[#0B3B4A] after:scale-x-100"
      : "text-[#5c5c5c] after:scale-x-0";
  ...
  const mobileLinkStyle =
    "block py-3.5 px-6 text-[12.5px] tracking-[.08em] uppercase text-[#5c5c5c] font-medium border-b border-[#DDD8D0] hover:text-[#0B3B4A] hover:bg-[#FAF8F4] transition-colors";
  ...
  return (
    <>
      <nav className="fixed top-0 left-0 right-0 z-[100] bg-white border-b border-[#DDD8D0] h-[72px] flex items-center justify-between px-4 sm:px-5 md:px-8 lg:px-12 font-['Inter',sans-serif]">
        <Link
          to="/"
        ...
        >
          ...
          <div className="font-['Cormorant_Garamond',serif] text-[13px] sm:text-[14.5px] font-semibold text-[#0B3B4A] tracking-[.03em] leading-[1.1] truncate">
        ...
        <div className="hidden xl:flex items-center">
            ...
            <div className="absolute hidden group-hover:block top-full left-0 bg-white border border-[#DDD8D0] border-t-[2px] border-t-[#C4912A] min-w-[272px] z-[200] shadow-[0_8px_28px_rgba(0,0,0,.1)]">
              {services.map((svc, i) => (
                <Link
                  key={i}
                  to="/services"
                  className="flex flex-col py-3 px-5 text-[12px] text-[#5c5c5c] border-l-[3px] border-transparent border-b border-[#DDD8D0] last:border-b-0 hover:text-[#0B3B4A] hover:border-l-[#C4912A] hover:bg-[#FAF8F4] transition-all"
                >
                  {svc.title}
                  <small className="text-[11px] text-[#999] mt-[2px]">
                    {svc.sub}
                  </small>
                </Link>
              ))}
          ...
          <Link
            to="/contact"
            className="bg-[#0B3B4A] hover:bg-[#082e3a] text-white py-2 px-[18px] lg:px-[22px] ml-2 lg:ml-3.5 tracking-[.09em] text-[11px] lg:text-[11.5px] font-medium uppercase transition-colors whitespace-nowrap"
          >
            Get in Touch
          </Link>
        </div>
        ...
        >
          <span
            className={`w-[22px] h-[1.5px] bg-[#0B3B4A] block transition-transform ${
              menuOpen ? "translate-y-[6.5px] rotate-45" : ""
            }`}
          />
          <span
            className={`w-[22px] h-[1.5px] bg-[#0B3B4A] block transition-opacity ${
      ...
      {menuOpen && (
        <div className="xl:hidden fixed top-[72px] left-0 right-0 bg-white border-b border-[#DDD8D0] z-[99] shadow-[0_6px_20px_rgba(0,0,0,.08)] max-h-[calc(100vh-72px)] overflow-y-auto">
          <div className="py-2">
            {navItems.map((item) => (
              <Link
                key={item.path}
            ...
            <Link
              to="/contact"
              className="block mx-5 my-4 text-center bg-[#0B3B4A] hover:bg-[#082e3a] text-white py-3 px-5 tracking-[.09em] text-[12px] font-medium uppercase transition-colors"
              onClick={closeMenu}
            >
              Get in Touch
            </Link>
